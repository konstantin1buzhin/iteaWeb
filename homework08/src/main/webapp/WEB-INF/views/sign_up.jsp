		...
		<tr>
			...
			<c:if test="${param['login'] != null}">
				...
				<c:if test="${fn:length(param['repassword'])== 0}">
					<c:set var="existError" value="true" />
					<c:set var="errorText"
						value="${errorText}<li>Empty Re-Password</li>" />
				</c:if>

				<c:if test="${!param['re-password'] eq param['password']}">
					<c:set var="existError" value="true" />
					<c:set var="errorText"
						value="${errorText}<li>Passwords aren`t equals</li>" />
				</c:if>

				<c:if test="${param['gender']== null}">
					<c:set var="existError" value="true" />
					<c:set var="errorText"
						value="${errorText}<li>Choose your gender</li>" />
				</c:if>

				<c:if test="${param['address'] eq 'Choose'}">
					<c:set var="existError" value="true" />
					<c:set var="errorText"
						value="${errorText}<li>Choose your address</li>" />
				</c:if>
				...
				<c:if test="${param['amigo']== null}">
					<c:set var="existError" value="true" />
					<c:set var="errorText"
						value="${errorText}<li>Confirm installation AmigoBrowser</li>" />
				</c:if>
				...
				<td>
					<form id="loginForm" action='' method='post'>
						<table border='1'>
							<div class="field">
								<tr>
									<td align='center'><label>Login(e-mail)</label></td>
									<td align='center'>
										<div class="input">
							...
							<div class="field">
								<tr>
									<td width='50' align='center'><label>Password</label></td>
									<td width='50' align='center'><input type='password'
										, name='password'
										pattern="(?=.*[0-9])(?=.*[A-Z]{2,})[0-9a-zA-Z]{8,}" /></td>
								</tr>
							</div>

							<div class="field">
								<tr>
									<td width='200' align='center'><label>Re-Password</label>
									</td>
							...
							<div class="field">
								<tr>
									<td width='50' align='center'><label>Age(12-100)</label></td>
									<td width='50' align='center'><input type='number'
										, name='age' min="12" max="100" value="${param['age']}" /></td>
								</tr>
							</div>

							<div class="field">
								<tr>
									<td width='50' align='center'><label>Gender</label></td>
									<td width='50' align='center'>F<input type='radio'
										...
										${((param['gender']!=null)&&(param['gender'] eq 'M')) ? 'checked' : ''} />
									</td>
								</tr>
							</div>
							...
							<div class="field">
								<tr>
									<td width='50' align='center'><label>Address</label></td>
									<td width='50' align='center'><select name='address'>
											...
											<option value='Crimea'
												${((param['address']!=null)&&(param['address'] eq 'Crimea')) ? 'selected' : ''}>Crimea</option></td>
								</tr>
							</div>

							<div class="field">
								<tr>
									<td width='50' align='center'><label>Comments</label></td>
									<td width='50' align='center'><textarea name='comments'
											cols='20' rows='10'>${param['comments']}</textarea></td>
								</tr>
							</div>

							<div class="field">
								<tr>
									<td width='50' align='center'><label>Amigo</label></td>
							...
							<input type='hidden' name='currentSession'
								value="${param['currentSession']}" />
							<input type="hidden" name="currentName"
								value="${param['currentName']}" />
						...
						<div class="submit">
							...
							<tr>
								...
								<td height='50' width='50' align='center'></td>
							</tr>
						</div>
					</form>
			</c:if>
