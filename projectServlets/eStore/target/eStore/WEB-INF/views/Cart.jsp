					...
					<c:forEach var="product" items="${sessionScope.CART}">
						<li class="product-wrapper"><c:set var="countProducts"
							...
							<div class="product">
								...
								<a href="/eStore/ProductController?id=${product.key.id}"><div
										class="product-photo">

										<img height="${imgHeight}"
											src="/eStore/static/images/${product.key.id}.jpg" alt="" />
									</div></a>
								...
								<img width='25px' height='25px'
									src="/eStore/static/images/plus.png"
									onclick='plus("${product.key.id}")' /> <br>
								<input type='button' value='Details' style="color: blue;"
									onclick="javascript:location.href='/eStore/ProductController?id=${product.key.id}'" /> <br> <img
									align="left" height="30" width='30px'
									src="/eStore/static/images/bin.png"
	...
	<script>
		...
		function getProductById(id) {
			$.ajax({
				...
				success : function(data) { // вешаем свой обработчик на функцию success
					window.location.href = '/eStore/ProductController';
				}
			})
		}
		...
		function deleteProduct(id) {
			$.ajax({
				...
				success : function(data) { // вешаем свой обработчик на функцию success
					window.location.href = '/eStore/CartController';
				}
			})
		}
